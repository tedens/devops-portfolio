...
Project 04 ships dashboards. Dashboards are for after you have been woken up.
This is the part that wakes you: two SLOs for the service delivered by
project 16, multi-window multi-burn-rate alerts on their error budgets, a
routing tree that separates a page from a ticket, one runbook per alert
linked from the alert itself, and a drill that burns the budget on purpose
and proves the page arrives, the redundant ticket is held, and the page
clears when the burn stops.

Every rule is unit-tested offline. The tests are themselves tested: CI breaks
the thresholds and fails if the suite stays green.

## Run it

```bash
./scripts/test.sh        # promtool + amtool + runbook checks, seconds, no cluster
./scripts/install.sh     # into project 16's kind cluster: rules, Alertmanager, sink
./scripts/burn-drill.sh  # burn, page, hold the ticket, clear
```
...
| latency | `requests slower than 250ms / total` | 99% | 7.2 hours over threshold |

Both are "error ratios" because the alerts compare them against a budget,
and a budget is an allowance of errors, not of successes. The objectives are
recorded as series (`slo:objective:ratio`) so dashboards and alerts read one
number from one place, and adding a third SLO is two recording rules and a
runbook, not a new alert.
...
across pods; a fleet-wide p95 from them is not a p95 of anything. Buckets
can be summed.
...
Multi-window, multi-burn-rate, from the SRE workbook. A burn rate of 1 spends
exactly the budget in 30 days.

| alert | pairs | severity | for |
|---|---|---|---|
...
minutes of the error ratio dropping. The drill checks that.

`NoTraffic` and `ServiceDown` exist because of what the burn alerts cannot
see. No traffic means the SLI is `0/0`, which is NaN, and every comparison
with NaN is false: silence. That is correct (no traffic is not a budget
burn) and dangerous (an outage that drops traffic to zero looks like a quiet
day). Adding `or vector(0)` to the SLI would hide an outage behind a
comfortable 0% error rate; the two health alerts own that case instead.

## Routing

`severity=page` goes to the pager receiver with no grouping delay and a
one-hour repeat. `severity=ticket` goes to the ticket receiver. Anything
with no severity lands on ticket, because an unclassified alert paging
someone at 3am is how alerts get muted.

One inhibition rule: while a page fires for a service and SLO, the slow-burn
ticket for the same pair is suppressed. The ticket is not wrong; it is
redundant, and two notifications for one problem teaches people to skim.

Both receivers are webhooks to a small in-cluster sink that logs one JSON
line per alert, tagged with the receiver it arrived on. Swapping in
PagerDuty is a receiver change. The sink exists so the drill can read what
was *delivered*, which is a different fact from what Prometheus thinks is
*firing*.

## What is tested offline

`tests/slo_alerts_test.yaml`, via `promtool test rules`:

- a broken build at 60% errors pages within minutes, and does not also ticket
- healthy traffic at 0.05% errors is silent on all four alerts for three hours
- a 0.5% leak opens a ticket after a day and never pages
- no traffic leaves the SLI as NaN (asserted: not 0), burn alerts stay silent, NoTraffic fires
- every scrape failing pages, and does not also raise NoTraffic
- no targets at all also pages
- slow responses burn the latency budget with zero 5xx
- with seven hours of clean history, a 4-minute burn pages at 6 minutes and is gone 4 minutes after it stops

`scripts/test.sh` adds: `amtool check-config`, five routing cases through
`amtool config routes test`, and a bidirectional check that every alert has
`severity`, `summary` and a `runbook_url` pointing at a file that exists,
and that every runbook file corresponds to an alert.

CI then mutates both fast-burn thresholds to 100x and requires the suite to
go red. The first version of that mutation raised only the 14.4x pair and
the suite stayed green, correctly: with a fresh 60% burn the 6x pair fires
too, because a ratio of two rates over the same window is 0.6 however little
of the window holds data. A mutation the alert can route around is not a
mutation.
...
One per alert, in `runbooks/`, linked from the alert's `runbook_url`. They
are written for the person who was just woken up: what it means, the three
queries to run first, what to do if one version is at fault (Argo Rollouts
should already have aborted; check whether it did), and what to write down
afterwards.

## Layout

```
rules/               recording rules (7 windows x 2 SLOs) and the four alerts
tests/               promtool unit tests
alertmanager/        routing, inhibition, receivers
runbooks/            one per alert
manifests/           Alertmanager and the notification sink
kustomization.yaml   rules and routing become ConfigMaps from the tested files
scripts/             test.sh, install.sh, burn-drill.sh
```

The kustomization sits at the project root, not in `manifests/`, because
kustomize refuses to read files above its directory and Argo CD builds with
that restriction on. Project 16's Prometheus mounts the rules ConfigMap by
name as an optional volume, so 16 runs on its own before 18 is installed;
`16-gitops-delivery/apps/slo-alerting.yaml` is the Argo CD Application that
delivers this directory.
